...
泊まれる宿の空室を定期取得し、**新規に空きが出た宿だけ**通知する。
対象日程は次の 4 通り（`config.toml` の `[[stay.ranges]]`）。本命は決勝翌日に帰る **金〜月**。

| チェックイン | チェックアウト | 泊数 | 備考 |
|---|---|---|---|
| 4/9(金) | 4/12(月) | 3 | 本命 |
| 4/9(金) | 4/11(日) | 2 | 決勝日に帰る |
| 4/8(木) | 4/11(日) | 3 | 前日入り |
| 4/10(土) | 4/12(月) | 2 | 予選日は行かない |

人数は次の 3 パターンを同時に検索する。

| ラベル | 内訳 | 部屋数 | 「即」しきい値（1泊・全室合計） |
|---|---|---|---|
| 親子2人1室 | 大人1＋4歳児1（添い寝） | 1 | 25,000 円 |
| 4人1室 | 大人3（本人＋祖父母）＋4歳児1 | 1 | 60,000 円 |
| 4人2室 | 同じ4人 | 2 | 60,000 円 |

パターンは `config.toml` の `[[stay.parties]]` で追加・変更できる。通知と差分はパターンごとに独立している。
...
前提・圏の決め方・保留事項は [DECISIONS.md](DECISIONS.md) にまとめてある。
設定を変える前にそちらを読むと、なぜその値なのかが分かる。
...
東横INN は小学生以下 1 名の添い寝が無料。非会員でも 9 か月前から予約できるため、
津駅西口・近鉄四日市駅北口は最優先（2027/4 分は 2026/7 に開放済み）。

施設コードが未設定（`00000`）の間は、実行時に公式サイトを開いて候補を
`data/toyoko_candidates.txt` に書き出す。そこから正しいコードを `config.toml` に転記する。
手動で実行するなら `python monitor.py toyoko-discover`。

## 3. 定期実行（NAS）

### A. cron

`deploy/crontab.example` を参照。`config.toml` の `daily_times`（既定 7:00 / 12:00 / 18:00 / 22:30）に `run --notify`、毎時 5 分に `run --notify --only-if-dense`。
...
- **即／参考**: 1 泊あたり（全室合計）が人数パターンの `instant_price_per_night` 以下なら「即」、超えたら「参考」
...
- **料金の出し方**: 楽天の `dailyCharge.total` は「その日 1 泊・1 室あたりの合計」。日別が泊数分返れば合算し、
  初日分しか返らない場合は 1 泊単価 × 泊数で概算して表と通知に `≈` を付ける。2 室なら部屋数ぶん倍にする。
  最終的な金額は必ず予約ページで確認すること
...
- 楽天のリクエスト数はエリア数 × 日程 × 人数パターン × ページ数（既定 30 件/ページ）。8 エリア × 4 日程 × 3 パターン ≒ 96 req/回（1 req/sec なので約 2 分）
